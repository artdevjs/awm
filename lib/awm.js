...
var downloadFile = function(wf, callback) {
  ...
  fs.exists(filePath, function(exists) {
    if (exists) {
      ...
      }).pipe(fs.createWriteStream(filePath)).on('close', function(err) {
        if (err) 
          ...
          verifySignature(wf, filePath, function(valid) {
            if (valid) {
              ...
              fs.remove(filePath, function(delErr) {
                if (delErr) 
                  console.error(('Cannot delete workflow file: ' + filePath).red);
...
var getAlfredPreference = function(callback) {
  var alfredPref = process.env.HOME + '/Library/Preferences/com.runningwithcrayons.Alfred-Preferences-3.plist';
  var alfredNewStyle = process.env.HOME + '/Library/Preferences/com.runningwithcrayons.Alfred-Preferences-3-NewStyle.plist';
  ...
  exec('plutil -convert xml1 ' + alfredPref + ' -o ' + alfredNewStyle, function(error) {
    if (error) {
      ...
      var location = (prefFile.syncfolder === undefined)
        ? process.env.HOME + '/Library/Application Support/Alfred 2'
        : prefFile.syncfolder.replace('~', process.env.HOME);
...
function splitArrayValues(jsManifest) {
  _(jsManifest.manifest.workflow).each(function(wf) {
    if (wf.categories) 
      wf.categories = _.map(wf.categories.split('|||'), function(str) {
        return str.trim();
      });
    if (wf.osx) 
      wf.osx = _.map(wf.osx.split('|||'), function(str) {
        return str.trim();
      });
    if (wf.tags) 
      wf.tags = _.map(wf.tags.split('|||'), function(str) {
        return str.trim();
      });
    if (wf.webservices) 
      wf.webservices = _.map(wf.webservices.split('|||'), function(str) {
        return str.trim();
      });
    }
  );
      ...
      parser.parseString(body, function(err, jsResult) {
        if (err) 
          ...
          var file = fs.readFile(filePath, function(err, data) {
            if (err) 
              callback('Cannot read downloaded file');
            else {
